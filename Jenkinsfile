        ...
        stage('Run Unit Tests on VM2') {
            steps {
                script {
                    ...
                    sshagent([VM2_SSH_CREDENTIALS]) {
                        sh """
                            ssh -o StrictHostKeyChecking=no now@${VM2_HOST} 'cd /path/to/simple-api && python3 -m venv venv && source venv/bin/activate && pip install -r requirements.text && pytest test/'
                        """
                    }
                }
            }
